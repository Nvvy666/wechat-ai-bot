...
### 方案一: Windows Sandbox (推荐，Win 10/11 Pro)

```
双击 sandbox.wsb → 沙盒桌面自动打开
→ 在沙盒里: 安装微信 + 双击 setup.bat + 启动 Bot
→ 你的主桌面完全不受影响
```
...
- ✅ 完全隔离，关闭沙盒自动清除
- ✅ 项目文件夹自动映射到沙盒桌面
- ⚠️ 需要 Win 10/11 **Pro** 或 Enterprise（家庭版需先开启）

**开启 Sandbox**: 控制面板 → 启用或关闭 Windows 功能 → 勾选 "Windows 沙盒"

### 方案二: 双用户切换

```
Win+L → 切换到另一个 Windows 用户
→ 在第二个用户里: 登录微信 + 启动 Bot
→ Win+L 切回主用户继续工作
```
